...
- 配置层：统一使用挂载到容器内的 `yuiju.config.json`。需要从环境变量读取的字符串配置必须显式写成 `{ "$env": "变量名" }`。
...
- `world` 报数据库连接失败：
  - 检查挂载进容器的 `yuiju.config.json` 是否使用了容器网络地址：
...
- 构建阶段和运行阶段都会传入若干 `YUIJU_*` 变量；只有被 `yuiju.config.json` 中 `$env` 引用的变量会进入最终配置。
- 运行阶段会挂载仓库根目录的 `yuiju.config.json`。容器连接 MongoDB / Redis 时，必须确保该文件中的地址适合容器网络。
