...
Planner Session 应写入 Redis，与会话实时状态保持一致。进程启动时与最近真实聊天消息使用同一个 5 分钟恢复窗口：`updatedAt` 距启动时间不超过 5 分钟的 Planner Session 保留，超过窗口的 Session 从 Redis 删除。恢复已经完成的 Planner 上下文和压缩摘要，但不恢复正在执行的 LLM 请求、分段发送、待处理消息、退避状态或定时器，也不补发重启前未发送的回复。
...
完整人物画像只提供给 Planner。Planner 在 `reply_context` 中汇总本次回复真正有用的人物关系、称呼、雷区或近期互动；Replyer 不接收画像原文，避免把人物分析直接说出口。
...
悠酱成功发送 `reply` 文字或 `sendSticker` 后进入 15 秒关注期。关注期内任意一条新消息立即调度 Planner，同样绕过普通必要性门控与连续无行动退避。只有真实发送成功才刷新关注期；Planner 沉默、`poke`、查询工具、计划变更和收到群友消息都不延长关注期。最后发言时间直接从共享群聊历史确定，不建立第二份持久状态。
...
- 选择并调用本次判断需要的工具。
- 收集可能的计划变更提案。
- 在回复时指定目标消息、引用行为，并为 Replyer 准备完整回复上下文。

Planner 不直接生成能够发送的角色台词，也不只是输出一次性的 `shouldReply` 分类结果。Planner 通过工具表达行动：

```text
reply(targetMessageId, set_quote, reply_context, stickerIntent?)
sendSticker()
...
`reply` 参数不包含成品台词，但包含 Planner 为本次回复准备的完整自然语言上下文：

- `targetMessageId` 明确主要承接哪条真实消息，避免泛泛回应整段历史。
- `set_quote` 与 MaiBot 语义一致，明确本次回复的第一条消息是否引用 `targetMessageId`；后续文字分段和附带表情包不重复引用。
- `reply_context` 是生成本轮回复所需的最小充分上下文：完整指不遗漏必要信息，不是复述所有可获得的信息。Planner 从回复目标出发逐项筛选；删除某项不会改变事实正确性、人物关系判断、对话连续性或世界边界时，就不写入。
- `reply_context` 只包含当前对话焦点、相关事实、确定的人物关系、仍有效的互动连续性、明确的内容立场和必须遵守的行动边界。内容立场只说明是否接受、拒绝、认同、反对、愿意或在意，不设计这种立场应当如何表现。
- 表达性情绪、人格标签和内心表演不进入 `reply_context`。Planner 不描述组织语言、选择内容、呈现情绪或准备回复的过程。普通中性或正面的表达优先按字面和善意理解，没有明确冒犯证据时不建立对抗立场。
- 人物身份与关系只能来自按 nickname 确定性注入的画像或聊天中已经明示的信息，Planner 不根据名字相似、称呼习惯或角色设定猜测身份映射。查询工具结果只提炼不可缺少的结论，不复述原始结果。
- `reply_context` 使用普通自然语言，不使用结构化格式，不裁剪长度。
- `reply_context` 可以说明是否认同、在意或误会了对方，但不能规定成品措辞、开场、口吻、句式、消息长度或分步表达，也不描述 Planner 准备怎样接话、组织答案或向对方呈现情绪。
- 需要在文字后补一个表情包时，额外提供自然语言 `stickerIntent`，只描述希望表达的反应，不填写具体资源 key；不需要表情包时不提供。

Planner 负责把分散在真实聊天、人物画像、持续上下文和工具结果中的有用信息整理进 `reply_context`，但不把它写成 Replyer 必须逐项执行的内容提纲。Replyer 根据回复目标和完整上下文决定最终只说哪一小部分。

Planner 不注入完整角色设定，避免它把内向、小别扭等表达人格提前扩写成回复心理剧本。Planner 仍通过聊天行为规则识别自己的消息身份，并接收完整当前角色状态、世界设定、相关人物画像、真实聊天和持续 Session；需要最近聊天和当前状态之外的日记、人物经历、世界、计划或物品事实时，先调用现有查询工具，再将结果中有用的部分写入 `reply_context`。程序不自动向 Replyer 透传原始 Tool Result；行动工具和计划提案结果同样不进入 Replyer。`reply_context` 只存在于本次行动和 Planner Session 的 Tool Call 中，不建立新的持久状态。
...
Planner 使用现有 `getFlashModel()` 并开启 thinking，使它在选择行动和准备最小充分 `reply_context` 前完成相关性判断；Replyer 使用现有 `getChatModel()` 并开启 thinking，优先保证最终可见回复的人格一致性和表达效果。不新增模型档位，也不改变模型选择顺序；模型来源可通过 `context_window_tokens` 覆盖默认的 128k Context Window。模型—工具循环在同一个 Planner Session 中继续，轮数上限首先服务于完成有效社交判断，不沿用旧链路的调用方式作为约束。
...
- 完整角色设定；新策略中只有 Replyer 使用其中的表达人格。
- 世界设定。
- 当前心情。
- 最近真实聊天历史。
- 当前待处理消息批次。
- Planner 指定的目标消息。
- Planner 为本次回复准备的完整 `reply_context`。
- 回复表达规则。

Replyer 不接收：

- 持续 Planner Session、Planner 的分析过程、`wait` 记录和无行动判断。
- 工具定义、Tool Call、原始 Tool Result 和计划提案结果。
- 内部触发分数、任务版本和批次调度信息。

Replyer 使用现有 `getChatModel()` 并开启 thinking 生成普通文本，不使用 structured output，不输出 JSON、`[[sticker:key]]` 或人为控制消息条数的特殊分隔符。模型只负责“怎么说”，并以稳定人格与相处原则作为最终口吻约束；发送层负责“分成几条发送”，表情包选择器负责“用哪张图”。

Replyer 的表达规则以消除助手式组织方式为核心：

- Replyer 直接回应目标，只说当下最值得说的一个重点。普通群聊默认一句话；只有第二个意思对当前回应确实不可缺少时才增加一句。
- `reply_context` 只是内容依据，Replyer 不复述其中的内部分析，也不向对方说明自己组织语言、选择内容或生成答案的过程。
- Replyer 的人格完全来自角色设定，不额外强化或表演少女感、小别扭、害羞、嘴硬等特征。普通中性或正面的表达按字面和善意回应，没有明确冒犯时不自行建立防御立场。
- 不默认提供帮助、建议、选项或完整解决方案。
- 对方表达低落时只回应最贴近的一个点，不同时完成复述、安慰、建议、承诺和追问。
- 不复述对方的话来证明自己理解了上下文。
- 不站在旁观者角度总结他人的性格、关系、情绪或聊天氛围。
- 可以只接半句话、只表达一个偏好、只吐槽一个点，也可以不把话说圆。
- 不使用“复述 + 表态 + 建议 + 问题”的完整助手结构。
- 只有对方明确要求详细解释、操作步骤或完整方案时才充分展开信息。
- 自我经历只能来自 `reply_context` 或最近聊天中的已确认内容，不能为了共情临时编造经历。
- `reply_context` 是完整内部参考，不是必须逐项说出的素材；人物画像、记忆、状态、世界和计划事实只有在目标消息直接询问或自然回应确实需要时才使用。

Replyer 不注入场景—答案式表达范例，避免模型模仿测试答案代替泛化判断。首轮 Replyer 固定读取共享历史中的最近 20 条真实消息，当前待处理批次已经包含在其中时不重复拼接。
...
- Planner 本次的表情反应意图；独立表情包行动没有显式意图参数时，使用 Planner 调用该行动前的当前判断。
...
| Planner Session | Redis | 保存压缩后的主观连续性和最近完整逻辑轮次；启动时与真实聊天共用 5 分钟恢复窗口，过期后删除；它是内部行动状态，不替代真实聊天。 |
...
| 回复后关注期 | 由共享群聊历史中的最后一次真实回复时间即时计算 | 不单独持久化；发送成功后 15 秒内的消息绕过普通必要性门控。 |
...
- 成功发送真实回复后的 15 秒关注期内，任意一条新消息立即触发 Planner；Planner 沉默不会刷新关注期。
...
- Replyer 只能看到真实聊天、回复目标、当前心情和 Planner 汇总的完整 `reply_context`，看不到持续 Planner 上下文、完整人物画像和原始 Tool Result。
- Planner 与最近真实聊天使用相同的 5 分钟启动恢复窗口；超过窗口的 Planner Session 会从 Redis 删除。
