...
- 当前对话焦点和 Planner 的真实主观立场通常是核心；人物关系与称呼、Planner Session 中的互动连续性、相关记忆、当前角色状态、世界与计划事实仅在确实影响本次回复时加入，不要求覆盖所有类别。当前状态只有在对方直接询问、影响能否做某事或用于避免事实冲突时才进入 `reply_context`。
...
- `reply_context` 可以说明是否认同、在意或误会了对方，但不能规定成品措辞、开场、口吻、句式、消息长度，不能使用“毒舌地”“小别扭地”“温柔地”等方式指令替 Replyer 设计人格。
- `reply_context` 不安排“先……再……”“顺带提……”等回复步骤，避免重新成为隐式的表达提纲。
...
Planner 默认输入包含完整当前角色状态，使它能够判断哪些生活事实可能影响本次回复。需要最近聊天和当前状态之外的日记、人物经历、世界、计划或物品事实时，Planner 先调用现有查询工具，再将结果中有用的部分写入 `reply_context`。程序不再自动向 Replyer 透传原始 Tool Result；行动工具和计划提案结果同样不进入 Replyer。`reply_context` 只存在于本次行动和 Planner Session 的 Tool Call 中，不建立新的持久状态。
