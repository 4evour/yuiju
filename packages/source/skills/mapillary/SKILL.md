...
- 只能使用 `scripts/mapillary_client.py` 查询地点和街景图片数据。
- 不要调用 LLM 搜索、网页搜索、地图服务或其他外部工具补充查询结果。
- 每次用户请求只能执行一次查询命令：
  - 随机云旅游时，只执行一次 `random-japan`。
  - 指定地点云旅游时，只执行一次 `search-address`。
- 不要因为结果为空、图片少、地点不够理想或想找更好图片而继续尝试其他关键词、扩大范围、改用随机探索或追加第二次脚本调用。
- Python 查询命令报错、异常退出或返回无法使用的数据时，整个 Skill 执行立即失败并结束。不要排查或修复脚本，不要修改参数重试，不要换用其他命令、工具或数据来源强行完成任务。
...
- 返回 `error` 或 `images` 为空时，告诉用户该地点附近暂时没有可用街景，或本次随机探索没有找到合适图片，不要继续查询。
