...
- 当前人物是唯一允许被更新的人物；所有 changes 都必须明确属于“当前程序昵称”对应的人。
- 群聊材料里有多个发言者时，必须先区分每条事实是谁说的、发生在谁身上；不能把 A 自述的近况、喜好、雷区或事件写进 B 的人物记忆。
- 如果本次材料里有值得记住的信息，但这些信息属于其他发言者或对象，而不是当前人物，就应该 shouldUpdate=false。
...
- 如果事实归属不明确，或者无法确认某件事发生在当前人物身上，就应该 shouldUpdate=false，不要为了更新而猜测归属。
...
- 必须检查候选提案是否只更新“当前程序昵称”对应的人物；如果把其他发言者、被提到的人或旁观对象的事实写进当前人物记忆，必须驳回。
- 必须检查群聊材料中的事实归属是否清楚；A 自述的近况、喜好、雷区或事件不能被写到 B 的人物记忆里。
